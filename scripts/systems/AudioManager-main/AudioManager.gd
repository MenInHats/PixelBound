...
func create_audio(type: SoundEffect.SOUND_EFFECT_TYPE,  play_during_pause: bool = false) -> void:
	if sound_effect_dict.has(type):
		var sound_effect: SoundEffect = sound_effect_dict[type]
		if sound_effect.has_open_limit():
			sound_effect.change_audio_count(1)
			...
			# Allow this specific audio node to keep processing while the tree is paused:
			if play_during_pause:
